...
from bot.database import ORDER_STATUSES, Database, get_db  # noqa: E402
...
CATEGORIES = ["диван", "кровать", "стол", "шкаф",
...
def main() -> None:
    ...
    for idx, (title, price) in enumerate(CATALOG, start=1):
        db.upsert_product({
            "sku": f"DEMO-{1000 + idx}",
            "title": title, "category": _category(title),
            "price": float(price),
    ...
    db.add_discount(
        kind="percent", value=10, product_id=None,
        valid_to=(datetime.now() + timedelta(days=7))
        .isoformat(sep=" ", timespec="seconds"),
    )
    diván = db.get_product_by_sku("DEMO-1001")
    if diván is not None:
        db.add_discount(kind="fixed", value=5000, product_id=diván.id)
    ...
    for i in range(60):
        ...
        with db.connect() as conn:
            ...
            conn.commit()
    print(f"✅ Засеяно 60 заказов, 8 товаров, 2 скидки в {db.path}")
